...
//cases for very big from not solved
...
static int findSymbols(char c, char *text) {
    int i = 0;
    int number = 0;
    while (i != '\0') {
        if (text[i] == c) {
...
void formRichText (TB tb) {
    ...
    char *has2 = "</h1>";
    while (curr != NULL) {
        int length = strlen(curr->string);
        int numberOfAsterisk = findSymbols('*', curr->string) / 2;
        int numerOfHash = findSymbols('#', curr->string) / 2;
        int numberOfUnder = findSymbols('_', curr->string);
        int newLength = length + 7 * numberOfAsterisk + 9 * numerOfHash + 7 * numberOfUnder + 1;
        char *c = malloc(sizeof(newLength));
        char tmp[length + 1];
        strcpy(tmp, curr->string);
        while (tmp[i] != '\0') {
            if (tmp[i] == '*' && tmp[i + 1] != '*') {
...
void test08() {
    TB tb1 = newTB("line 01\nline 02\nline 03\nline 04\nline 05\nline 06\nline 07");
    TB tb2 = cutTB(tb1, 3, 6);
    int number = linesTB(tb1);
    printf("%d\n", number);
    displayNode(tb2);
    printf("======\n");
    displayNode(tb1);
